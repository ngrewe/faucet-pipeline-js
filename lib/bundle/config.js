...
// * `nodeEnv`, a string, that will be set as the value of process.node.NODE_ENV in the
//   resulting bundle. It will also be used to set a variable `__DEV__` to true if it is
...
function generateConfig({ extensions = [], // eslint-disable-next-line indent
		...
		sourcemaps, sourcemap, compact, nodeEnv }, { browsers }) {
	...
	if(nodeEnv) {
		let replace = loadExtension("@rollup/plugin-replace",
				"failed to activate process.env.NODE_ENV replacement",
				"faucet-pipeline-nodeenv");
