...
function generateConfig({ extensions = [], // eslint-disable-next-line indent
	...
	if(esnext || jsx) {
		...
		activateTranspiler("./babel", transpiler, { browsers }, plugins, extensions);
	}
	if(typescript) { // XXX: duplicates Babel's section above
		activateTranspiler("./typescript", typescript, { browsers }, plugins, extensions);
	}
...
// XXX: awkward signature due to side effects
function activateTranspiler(modulePath, config, { browsers }, plugins, extensions) {
	let generateTranspiler = require(modulePath);
	let { plugin, extensions: ext } = generateTranspiler(config, { browsers });
